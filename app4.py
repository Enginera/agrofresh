...
# ==========================================
# 3. МАРШРУТИЗАЦИЯ СТРАНИЦ: СТРОГО 7 ВКЛАДОК
# ==========================================
...
# ФИКС ОПЕЧАТКИ С ИНДЕКСАЦИЕЙ ILOC ДЛЯ СТРИМЛИТ КЛАУД
...
if st.session_state.page == "Обзор":
    st.markdown(f"<div class='metric-card' style='background: #F1F8E9; border-left: 5px solid #2E7D32; margin-bottom: 25px;'><h2>Добро пожаловать! Объект анализа: {selected_field}</h2><p>Обзор текущего состояния сельскохозяйственных подсистем комплекса.</p></div>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown("<div class='metric-card'><b>Углеродная 🟢 нейтральность</b><br><h3>Прогресс на 85%</h3></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='metric-card'><b>Active Projects 🚜</b><br><h3>5 кампаний</h3></div>", unsafe_allow_html=True)
    with c3: st.markdown(f"<div class='metric-card'><b>Средняя эффективность 📈 (E)</b><br><h3>{avg_e_calculated:.4f}</h3></div>", unsafe_allow_html=True)
    
    st.markdown("### Состояние модулей")
    m1, m2, m3 = st.columns(3)
    with m1: st.markdown(f"<div class='metric-card'><b>Планирование 🌿 севооборота</b><br><small>{len(df_active)} агросроков</small><br><br><span class='status-badge status-success'>В норме</span></div>", unsafe_allow_html=True)
    with m2: st.markdown("<div class='metric-card'><b>Удобрения и 🪱 почва</b><br><small>Стабильная секвестрация</small><br><br><span class='status-badge status-success'>В норме</span></div>", unsafe_allow_html=True)
    with m3: st.markdown("<div class='metric-card'><b>Мониторинг защиты 🛡️ растений</b><br><small>Риски Rave рассчитаны</small><br><br><span class='status-badge status-success'>В норме</span></div>", unsafe_allow_html=True)
...
# Вкладка 3: УДОБРЕНИЯ И ПОЧВА
    ...
    with col_r: st.markdown(f"<div class='metric-card' style='height:125px;'><b>Показатели почвы слоев 🧪</b><br> Глубина пласта: {depth_value} | pH: 6.5 | Азот: 0.12%</div>", unsafe_allow_html=True)
...
# Вкладка 5: УРОЖАЙНОСТЬ И КАЧЕСТВО
    ...
    with uk3: st.markdown("<div class='metric-card'><b>Коэффициент качества</b><br><h2>92%</h2><span style='color:blue;'>Выше среднего</span></div>", unsafe_allow_html=True)
